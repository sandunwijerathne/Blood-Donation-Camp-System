...
$id = (int) ($_POST['id'] ?? 0);
if (!$id) sendJsonResponse(false, 'Invalid camp ID.');

try {
    $db = getDB();
    $stmt = $db->prepare("DELETE FROM blood_camps WHERE id = ?");
    $stmt->execute([$id]);
    sendJsonResponse($stmt->rowCount() > 0, $stmt->rowCount() > 0 ? 'Camp deleted.' : 'Camp not found.');
